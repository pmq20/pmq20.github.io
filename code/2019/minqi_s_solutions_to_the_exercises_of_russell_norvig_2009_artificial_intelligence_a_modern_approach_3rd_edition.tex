...
\item The Argument from Informality of Behavior. This objection does not carry weight because it is not necessarily true that the behavior is predictable once the laws/rules governing it have been understood. Moreover, such laws/rules are hardly captured by exhaustion via scientific observations. Turing's argument is valid because he attacks the argument by showing that it is confusing ``laws of behavior'' with ``rules of conduct'' and shows that ``laws of behavior'' can be hardly exhausted and does not lead to predictable behaviors even if they can.
\end{enumerate}
