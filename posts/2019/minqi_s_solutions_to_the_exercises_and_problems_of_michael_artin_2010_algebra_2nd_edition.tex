...
\title{Minqi's Solutions to the Exercises and Problems of Michael Artin (2010), Algebra (2nd Edition)}
...
\section{Exercise 1.1.1: The Row Index and the Column Index}
...
\section{Exercise 1.1.2: Matrix Multiplication is not Commutative}
...
\section{Exercise 1.1.3: Row $\times$ Column and Column $\times$ Row}
...
\section{Exercise 1.1.4: Matrix Multiplication is Associative}
...
\section{Exercise 1.1.5: The Number of Multiplications Required to Multiply 3 Matrices}
...
\section{Exercise 1.1.6: $2\times2$ Upper Triangular Matrices}
...
\section{Exercise 1.1.7: $3\times3$ Upper Triangular Matrices}
...
\section{Exercise 1.1.8: Block Multiplications}
...
\section{Exercise 1.1.9: Laws of Matrix Operations}
...
\section{Exercise 1.1.10: Multiplications with Diagonal Matrices}
...
\chapter{Groups}
\section{Exercise 2.1.1: The Law of Composition Defined by $ab=a$ for all $a$ and $b$}
...
\begin{proof}
Pick any $a,b,c\in S$. Then\[
(ab)c=ac=a
\]and\[
a(bc)=ab=a.
\]Hence this law of composition is associative.

We claim that the sets, for which this law have an identity, are the ones that contain exactly one element. To prove this, suppose there exists an identity $e\in S$. Then\[
e=ea=a,\quad\forall a\in S.
\]Hence the set contains exactly one element $e$, i.e.\[
S=\{e\}.
\]
\end{proof}

\section{Exercise 2.1.3: The Shift Map has NO Right Inverse but Infinite Left Inverses}
Let $\mathbb{N}$ denote the set $\{1,2,3,\dots\}$ of natural numbers, and let $s:\mathbb{N}\to\mathbb{N}$ be the shift map, defined by $s(n)=n+1$. Prove that $s$ has no right inverse, but it has infinitely many left inverses.
\begin{proof}
Suppose $s$ has a right inverse $t:\mathbb{N}\to\mathbb{N}$, then\[
s(t(n))=t(n)+1=n,\quad\forall n\in\mathbb{N}.
\]In particular,\[
t(1)+1=1.
\]But\[
t(1)+1\geqslant1+1=2,
...
Let $r_k:\mathbb{N}\to\mathbb{N}$ be defined by\[
r_k(n)\equiv\begin{cases}
k & \text{if } n=1,\\
n-1 & \text{otherwise},
\end{cases}
\]where $k\in\mathbb{N}$. Pick any $n\in\mathbb{N}$, then\[
r_k(s(n))=r_k(n+1)=n.
\]Hence $r_k$ is a left inverse of $s$. Further, since there are infinitely many choices of $k$ in $\mathbb{N}$, $s$ has infinitely many left inverses.
\end{proof}

\section{Exercise 2.2.1: The Multiplication Table of $S_3$}
Make a multiplication table for the symmetric group $S_3$.
\begin{proof}[Answer]
Let $x,y\in S_3$ be defined by\[
\begin{array}{ccc}
x(1)=2,&x(2)=3,&x(3)=1;\\
y(1)=2,&y(2)=1,&y(3)=3.
\end{array}
\]Then\[
\begin{array}{ccc}
xx(1)=x(2)=3,&xx(2)=x(3)=1,&xx(3)=x(1)=2;\\
yy(1)=y(2)=1,&yy(2)=y(1)=2,&yy(3)=y(3)=3;\\
xy(1)=x(2)=3,&xy(2)=x(1)=2,&xy(3)=x(3)=1;\\
yx(1)=y(2)=1,&yx(2)=y(3)=3,&yx(3)=y(1)=2.\\
\end{array}
\]Hence\[
S_3=\{x,y,xx,yy,xy,yx\}.
\]
...
\section{Exercise 2.2.2: The Subset of Invertible Elements is a Group}
...
\begin{proof}
Denote the identity element of $S$ as $e$.

Define\[
G\equiv\{x\in S: x\text{ is invertible}\}.
\]

Pick any $x,y\in G$, we claim that $xy\in G$. This can be proven by showing that $y^{-1}x^{-1}$ is the inverse of $xy$:\[
(y^{-1}x^{-1})(xy)=y^{-1}(x^{-1}x)y=y^{-1}y=e,
\]\[
(xy)(y^{-1}x^{-1})=x(yy^{-1})x^{-1}=xx^{-1}=e.
\]

Next, we claim that $e\in G$. This can be proven by showing that $e$ is the inverse of $e$:\[
ee=e.
\]

Finally, we claim that $x^{-1}\in G$ if $x\in G$. This is obvious since $x$ is the inverse of $x^{-1}$.

Therefore, $G$ is a group.
\end{proof}

\section{Exercise 2.2.4: Examples and Counterexamples of Subgroups}
...
\item $G=GL_2(\mathbb{R})$ and $H$ is the set of matrices $\begin{bmatrix}
a&0\\
0&0
\end{bmatrix}$, with $a\ne 0$.
\end{enumerate}
\begin{proof}
\begin{enumerate}
\item We claim that $GL_n(\mathbb{R})$ is a subgroup of $G=GL_n(\mathbb{C})$. This can be shown by noticing that $\mathbb{R}$ is a field closed under multiplication and addition; the identity matrix is a real matrix; and the inverse of a real matrix $A$ is a real matrix because\[
A^{-1}=\frac{1}{\det A}\text{adj}(A).
\]and the determinant and minors of $A$ are all real.
\item We claim that $\{1,-1\}$ is a subgroup of $\mathbb{R}^\times$. To prove this, we first check the closure:\[
\begin{split}
1\times1&=1\in H,\\
1\times(-1)&=-1\in H,\\
(-1)\times1&=-1\in H,\\
(-1)\times(-1)&=1\in H.
\end{split}
\]Then, note that the identity $1$ is in $H$. Finally, $1^{-1}=1\in H$ and $(-1)^{-1}=-1\in H$.
\item We claim that $H$ is not a subgroup of $\mathbb{Z}^+$. Because, say, the inverse of $8$ is not in $H$.
\item We claim that the set of positive reals is a subgroup of $G=\mathbb{R}^\times$. This can be shown by noticing that the product of two positive reals is a positive real; the identity $1$ is in $H$; and the inverse of a positive real is a positive real.
\item We claim that $H$ is not a subgroup of $GL_2(\mathbb{R})$, because $H\not\subset GL_2(\mathbb{R})$.
\end{enumerate}
\end{proof}
