...
\end{split}
\]We assume that $(\mathbf{X},\mathbf{Y})\sim P_{\lambda,p}$, where $P_{\lambda,p}$ is a probability distribution on $\mathbb{R}^{2n}$ according to which $(X_1,Y_1),\dots,(X_n,Y_n)$ are independent and identically distributed with a common distribution function $F_{\lambda,p}$, where\[
